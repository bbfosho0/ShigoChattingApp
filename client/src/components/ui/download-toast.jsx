...
const FileIcon = ({ type }) => {
  ...
  return (
    <div
      className={`relative flex-shrink-0 w-14 h-14 ${style.bg} rounded-lg flex items-center justify-center mr-4 overflow-hidden`}>
...
const ProgressBar = ({ progress }) => (
  <div
    className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-1.5 mt-1.5">
    <div
...
const ActionButton = ({ status, onCancel }) => {
  if (status === 'downloading') {
    return (
      <button
        onClick={onCancel}
        className="text-slate-500 dark:text-slate-400 font-semibold text-sm hover:text-slate-800 dark:hover:text-slate-200 transition-colors duration-200">Cancel
              </button>
    );
  }
  return (
    <button
      className="text-blue-600 dark:text-blue-500 font-semibold text-sm hover:text-blue-800 dark:hover:text-blue-400 transition-colors duration-200">Download
          </button>
  );
};
...
const FileItem = ({ file, onCancel }) => {
  ...
  return (
    <div className="flex flex-wrap items-center py-3">
      ...
      <div
        className="flex items-center justify-end w-full sm:w-auto mt-2 sm:mt-0 pl-[72px] sm:pl-0">
        <div
          className="flex-shrink-0 w-20 text-right text-sm font-medium text-slate-500 dark:text-slate-400">
          {displaySize}
        </div>
...
export default function App() {
  ...
  return (
    <div
      className="relative bg-slate-50 dark:bg-slate-900 min-h-screen w-full flex items-center justify-center font-sans transition-colors duration-300 py-8">
      <GridBackground />
      <ThemeToggle darkMode={darkMode} setDarkMode={setDarkMode} />
      <div
        className="relative w-full max-w-lg mx-4 sm:mx-auto bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 p-4 sm:p-8">
        <h1
          className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-4 px-2 sm:px-0">Your Downloads</h1>
        <div>
          {files.map(file => (
