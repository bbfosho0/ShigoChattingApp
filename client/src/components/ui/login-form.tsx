...
function hexToRgb(hex: string): [number, number, number] {
  const normalized = /^#[0-9a-fA-F]{6}$/.test(hex) ? hex : "#1E40AF";
...
export function SmokeyBackground({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      gl.uniform2f(
        iMouseLocation,
        hovering ? mouse.x * pixelRatio : displayWidth / 2,
        hovering ? (height - mouse.y) * pixelRatio : displayHeight / 2
      );
  ...
  return (
    <div className="w-full max-w-sm space-y-6 rounded-2xl border border-white/20 bg-white/10 p-8 shadow-2xl backdrop-blur-lg">
      ...
      <form className="space-y-8">
        ...
        <button
          type="submit"
          className="group flex w-full items-center justify-center rounded-lg bg-blue-600 px-4 py-3 font-semibold text-white transition-all duration-300 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-900"
        >
          Sign In
          <ArrowRight className="ml-2 size-5 transform transition-transform group-hover:translate-x-1" />
        </button>
        ...
        >
          <svg className="mr-2 size-5" viewBox="0 0 48 48" aria-hidden="true">
