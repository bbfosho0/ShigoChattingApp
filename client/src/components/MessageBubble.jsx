...
const AVATAR_COLORS = ["var(--sc-accent)", "#35627a", "var(--sc-success)", "#936b3d", "#8a4d63"];
...
const MessageBubble = ({
  ...
  const avatarColor = getAvatarColor(senderName || "U");
  ...
  return (
    <motion.div
      ...
      aria-label={`${isMine ? "Your" : senderName + "'s"} message${status !== "sent" ? `, ${status}` : ""}`}
    ...
    >
      {!isMine && !grouped && (
        <div
          ...
          style={{
            ...
            boxShadow: "0 2px 8px var(--sc-shadow)",
          }}
        >
      ...
      >
        ...
        <div className={`relative flex items-end gap-2 ${isMine ? "flex-row-reverse" : "flex-row"}`}>
          <AnimatePresence>
            {isMine && !isEditing && (hovered || focusWithin || actionOpen) && (
              <motion.div
              ...
              >
                <button
                  ...
                  className="sc-icon-button sc-touch-target h-10 w-10 rounded-lg"
                  type="button"
                ...
                <button
                  ...
                  className="sc-touch-target flex h-10 w-10 items-center justify-center rounded-lg"
                  style={{
          ...
          {isEditing ? (
            ...
            >
              <textarea
                ...
                rows={2}
                className="sc-scrollbar w-full resize-none bg-transparent text-[0.875rem] leading-relaxed sc-text-primary"
            ...
            >
              ...
              {message.updatedAt && message.updatedAt !== message.createdAt && (
                <span className="mt-0.5 block text-[0.63rem] sc-text-secondary">edited</span>
              )}
              {status === "pending" && <span className="mt-0.5 block text-[0.63rem] sc-text-secondary">sending…</span>}
              {status === "failed" && <span className="mt-0.5 block text-[0.63rem]" style={{ color: "var(--sc-danger)" }}>not sent</span>}
            </div>
          )}
        </div>
            ...
            <div className="flex items-center gap-2 pr-0.5">
              ...
              {!isEditing && (
                <button
                  ...
                  aria-controls={`message-actions-${message._id}`}
                >
                  <span aria-hidden="true">•••</span>
                </button>
              )}
            </div>
            {!isEditing && (actionOpen || forceActionRow) && (
              <div id={`message-actions-${message._id}`} className={`flex gap-2 ${forceActionRow ? "" : "md:hidden"}`} aria-label="Message actions">
                <button onClick={() => setIsEditing(true)} className="sc-icon-button sc-touch-target rounded-lg px-3 py-1 text-xs" type="button">
                  <Pencil size={13} aria-hidden="true" /> Edit
                </button>
                <button onClick={() => onDelete(message._id)} className="sc-touch-target rounded-lg px-3 py-1 text-xs" style={{ background: "transparent", border: "1px solid var(--sc-danger)", color: "var(--sc-danger)" }} type="button">
